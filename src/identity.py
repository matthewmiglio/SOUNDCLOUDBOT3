...
The SoundCloud handle this clone acts as is *per-clone state*, not source
code. It lives in data/profile.json next to the browser profile so the
tracked source files are byte-identical across clones -- only the
gitignored data/ directory differs.

Bootstrap:
    python src/main.py login          # interactive sign-in
    python src/main.py whoami         # scrape handle from authed browser -> profile.json
or:
    python src/main.py whoami --username bloodxo
"""
...
def set_username(username: str) -> None:
    """Persist the username into data/profile.json."""
    global _cached
